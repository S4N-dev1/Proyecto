    ...
    <div class="container my-5">
        ...
        <div class="p-3 mb-3 bg-primary text-white rounded-3">
            <div class="container-fluid py-2 d-flex justify-content-between align-items-center">
                <h1 class="display-6 fw-bold">Editar Provedor</h1>
                <a href="{{ route('provedor.index') }}" class="btn btn-light">Regresar</a>
            </div>
        </div>
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('provedor.update', $provedor->id_provedor) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <select name="id_persona" class="form-select" required>
                            ...
                            @foreach ($personas as $persona)
                                <option value="{{ $persona->id_personas }}"
                                    {{ $persona->id_personas == $provedor->id_persona ? 'selected' : '' }}>
